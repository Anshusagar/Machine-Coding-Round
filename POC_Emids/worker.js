  ...
  return new Promise((resolve, reject) => {
    https.get(url, (res) => {
      ...
      res.on("data", (chunk) => {
        ...
        // Calculate % if total known
        let percent = total ? ((received / total) * 100).toFixed(2) : null;
        const elapsed = (Date.now() - start) / 1000;
        const speed = (received / (1024 * 1024)) / elapsed; // MB/s
...
(async () => {
  const { id, name, url, type } = workerData;
  const dest = `./${name}.tmp`;

  try {
    const download = await downloadStream(url, dest, id);
    let result = {
      file: dest,
